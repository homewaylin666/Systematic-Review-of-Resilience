...
df <- df %>%
  mutate(disturbance_pattern = case_when(
    # Convert values with semicolons to "press" (n=14)
...
cv_full_analysis <- function(formula, data, 
                             cv_folds = 5, cv_repeats = 30, nperm = 50, mtry_fixed = NULL, 
                             mincriterion = 0, ntree = 1500,seed  = 207) {
  ...
  for (r in seq_len(cv_repeats)) {
    ...
    for (k in seq_along(folds)) {
      ...
      # Drop unused levels within each fold
...
cv_full_analysis <- function(formula, data, 
  ...
  for (r in seq_len(cv_repeats)) {
    ...
    for (k in seq_along(folds)) {
      ...
      # --- Fit model with retry logic to handle LAPACK SVD failures ---
      fit          <- NULL
      attempt      <- 0L
      ...
      while (is.null(fit) && attempt < 5L) {
        ...
        fit <- tryCatch(
          ...
          error = function(e) {
            cat(sprintf("  [r%d k%d attempt%d] cforest failed: %s - retrying...\n",
      ...
      # Skip fold if all 5 attempts failed
      if (is.null(fit)) {
        cat(sprintf("  WARNING: Skipping r%d_k%d after 5 failed attempts\n", r, k))
    ...
    if (r %% 5 == 0) cat(sprintf("Completed repeat %d/%d (skipped folds so far: %d)\n",
                                 r, cv_repeats, skipped_folds))
  }
  
  cat(sprintf("\nTotal skipped folds: %d / %d\n", skipped_folds, run_id))
...
final_results <- cv_full_analysis(
  fml, analysis_data,
  cv_folds = 5, cv_repeats = 30, nperm = 50, mincriterion = 0,
  mtry_fixed = 3, ntree = 1500, seed = 207
)
...
# Nicer labels (customize as needed)
pretty_names <- c(
