# M5 Figure 7: The relation between quantification and its important factors
rm(list=ls())
library(here)
setwd(here())
...
# Step 3: Find top 8 most frequent values after processing
...
# Step 4: Replace non-top-8 terms with "Others"
df <- df %>%
...
# Step 5: Apply new category modifications and capitalize first letters only
df <- df %>%
  mutate(final_quantification = case_when(
    # Merge recovery+resistance and other multidimensional
    final_quantification %in% c("recovery+resistance", "other multidimensional") ~ "Multidimensional\nquantification",
    # Merge tolerance and invariability into others
    final_quantification %in% c("tolerance", "invariability", "others") ~ "Others",
    # Rename inferred to Inference
    final_quantification == "inferred" ~ "Inference",
    # Capitalize only first letter of remaining categories
    final_quantification == "recovery time" ~ "Recovery time",
    final_quantification == "recovery speed" ~ "Recovery speed", 
    final_quantification == "recovery degree" ~ "Recovery degree",
    TRUE ~ paste0(toupper(substr(final_quantification, 1, 1)), substr(final_quantification, 2, nchar(final_quantification)))
  ))

# Step 6: Set the new order
...
counts1 <- df_unique.level %>%
  filter(!is.na(level)) %>%
  mutate(
    level = paste0(toupper(substr(level, 1, 1)), tolower(substr(level, 2, nchar(level)))),
...
counts1_less <- counts1 %>%
  mutate(final_quantification_less = case_when(
    final_quantification %in% c("Others") ~ "Others",
    final_quantification %in% c("Multidimensional quantification") ~ "Multidimensional",
...
counts2 <- df_unique.pattern %>%
  ...
  mutate(disturbance_pattern = paste0(toupper(substr(disturbance_pattern, 1, 1)), tolower(substr(disturbance_pattern, 2, nchar(disturbance_pattern))))) %>%
...
p2 <- ggplot(counts2, aes(x = disturbance_pattern, y = prop, fill = final_quantification)) +
  ...
  geom_line(
    ...
    aes(x = disturbance_pattern, y = total / 300 * 100, group = 1),  # Changed max(total) to 250
    inherit.aes = FALSE,
    color = "black", size = 0.5
  ) +
  geom_point(
    data = counts2 %>% distinct(disturbance_pattern, total),
    aes(x = disturbance_pattern, y = total / 300 * 100),  # Changed max(total) to 250
    inherit.aes = FALSE,
    color = "black", size = 1
  ) +
  ...
  # So we got two y axis with 250 as upper limit
  scale_y_continuous(
    name    = "Proportion (%)",
    sec.axis = sec_axis(
      ~ . * 300 / 100,  # Changed to use 250 instead of max(counts2$total)
      name = "Total Count"
    )
  ) +
...
counts3 <- df_unique.approach %>%
  ...
  mutate(
    approach = paste0(toupper(substr(approach, 1, 1)), tolower(substr(approach, 2, nchar(approach)))),
...
library(ggpubr)
# 1) Data prep: keep finite positives, drop "Inference", then log-transform.
counts4 <- df %>%
  filter(
    !is.na(final_quantification),
    final_quantification != "Inference",  # Updated name
  ...
  mutate(
    ...
    final_quantification = factor(final_quantification, levels = rev(quantification_order[quantification_order != "Inference"]))  # Remove Inference from levels
  )
...
target.group_order <- c("Quantity","Structure","Process based\nindicator","Functional\nresponse","Environmental\ncontext")
counts5 <- df_unique.target.group %>%
  filter(!is.na(target_variable_group)) %>%
  mutate(
    target_variable_group = paste0(toupper(substr(target_variable_group, 1, 1)), tolower(substr(target_variable_group, 2, nchar(target_variable_group)))),
    # Add line breaks for long names
    target_variable_group = case_when(
      target_variable_group == "Process based indicator" ~ "Process based\nindicator",
      target_variable_group == "Functional response" ~ "Functional\nresponse", 
      target_variable_group == "Environmental context" ~ "Environmental\ncontext",
...
counts5b <- df_unique.target.type %>%
  ...
  mutate(
    target_variable_type = paste0(toupper(substr(target_variable_type, 1, 1)), tolower(substr(target_variable_type, 2, nchar(target_variable_type)))),
...
counts6 <- df_habitat_type %>%
  ...
  mutate(
    habitat_type = paste0(toupper(substr(habitat_type, 1, 1)), tolower(substr(habitat_type, 2, nchar(habitat_type)))),
...
combined_plot <- (p.pattern + p.duration) /
  (p.target  + patchwork::plot_spacer())

print(combined_plot)
ggsave("Figure7.png", combined_plot, width = 3200, height = 2400,
       units = "px", dpi = 300)
...
legend_plot <- cowplot::ggdraw(legend_g)
ggsave("Figure7_legend.png", legend_plot,
       width = 700, height = 1600, units = "px", dpi = 300)
